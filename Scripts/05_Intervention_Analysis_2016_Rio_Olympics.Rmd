...
We suspect that the intervention can be described as ARMA(2,2) process.
...
# applying the model
page_views_mPulse <- arimax(log(page_2016_Rio_Olympics_views), order = c(1,1,1),
...
points(fitted(page_views_mPulse),col = 'red')

# plotting the actual and fitted in normal scale
plot(page_2016_Rio_Olympics_views)
points(exp(fitted(page_views_mPulse)),col = 'red')
```

Here we applied the Regression with ARMA error for capturing the underlying process and the intervention. We assumed that the transfer function that can represent the intervention is an ARMA(2,2) function and we used the coefficients got from here to plot the intervention below.
...
2. From ACF we can see that the residuals are not white noise because there is a significant spike at lag 7.  
